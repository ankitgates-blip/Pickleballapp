    ...
    <OrganizerShell organizerName={organizer.name} role={role}>
      ...
      {role !== 'owner' && (
        <p className="text-sm text-muted mb-6">
          📍 {venueName} · 🕐 {timeslotLabel(tournament?.timeslot ?? '')}
        </p>
      )}
        ...
        <div className={`${cardClass} mb-6`}>
          ...
          <form action={startAddPlayersWithId} className="space-y-3">
            <textarea
              ...
              required
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-navy-mid focus:border-navy-mid"
            />
            <SaveButton className={primaryButtonClass} pendingLabel="Adding…">
              Add Players
            </SaveButton>

# Player Stats Card v3 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an "Age · Handedness" line and a "Signature Shots" list to
the Player Stats Card, moving the shield to the right to make room and
relocating the "Record" stat into the side panel — the final design
approved through live mockup iteration.

**Architecture:** Two new props on `PlayerStatsCardProps`
(`ageHandednessLabel`, `signatureShots`), computed on the Player Detail
page from data it already fetches (no new query). The card component's
JSX is restructured: bottom stat row removed, shield's transform shifts
right, a new signature-shots text block fills the freed left space,
Record moves into the side panel's bullet list as its new first item.

**Tech Stack:** React (Client Component), inline SVG — unchanged from
v2.

## Global Constraints

- `ageHandednessLabel: string | null` — pre-formatted on the page (e.g.
  "Age 34 · Right-handed", just one part if only one is set, `null` if
  neither is set). The component does no formatting of its own.
- `signatureShots: { emoji: string; skillName: string }[]` — built on
  the page from the already-computed `signatureShotBadges` (used
  elsewhere on the same page), taking only `emoji`/`skillName` — no fun
  nickname on the card, no new query.
- Signature shots list shows short skill name only, italic, one per
  line, max 5 shown with a "+N more" line if there are more; the whole
  section (including the "SIGNATURE SHOTS" label) is omitted entirely
  when the player has none.
- Shield size/detail unchanged from v2 — only its horizontal position
  shifts right within the row.
- Record (W-L) removed from the main panel's bottom row (which is
  removed entirely) and added as the side panel's new first bullet.
- `PlayerStatsCardProps`' 13 existing fields are unchanged — this is
  additive (2 new fields), not a rename or removal of anything.

---

### Task 1: Restructure the card component and wire its two new props
...
**Interfaces:**
- Produces: `PlayerStatsCardProps` gains two new required fields:
  `ageHandednessLabel: string | null` and `signatureShots: { emoji:
  string; skillName: string }[]`.

This task adds two new REQUIRED props to `PlayerStatsCardProps` and
must land together with updating its only caller
(`people/[id]/page.tsx`) in the same commit — adding required props to
a shared component without updating its only caller in the same change
would leave the build broken between commits, which this project's
established convention avoids. `PlayerStatsCard.tsx`'s changes are a
full-file replacement (the header block, bottom stat row, shield
position, and side panel all move); the `page.tsx` changes are two
small find/replace edits. Rather than a sequence of fragile find/replace
edits for the component file, write its complete new content below.

- [ ] **Step 1: Replace the file**

Replace the entire contents of
`apps/organizer-web/app/components/PlayerStatsCard.tsx` with:
...
- [ ] **Step 2: Build `ageHandednessLabel` and `cardSignatureShots` on the Player Detail page**
...
- [ ] **Step 3: Pass the two new props to `<PlayerStatsCard>`**
...
```bash
git add apps/organizer-web/app/components/PlayerStatsCard.tsx "apps/organizer-web/app/people/[id]/page.tsx"
git commit -m "feat: add age/handedness and signature shots to Player Stats Card"
```
...
needed — this task touches no schema and no new query.
...
- Open a player who has age, handedness, AND signature shots all set.
  Confirm the card shows "Age N · Left/Right-handed" under the name,
  and a "SIGNATURE SHOTS" list with each shot's emoji + short skill
  name (no nickname), italic, positioned to the left of the shield —
  which should now sit toward the right side of that row instead of
  centered.
- Confirm "Record" (W-L) now appears as the FIRST item in the side
  panel's bullet list, and the main panel no longer has a bottom stat
  row below the shield/ribbon.
- Check a player with only ONE of age/handedness set — confirm only
  that one shows (e.g. just "Age 34", not "Age 34 · " with a trailing
  separator).
- Check a player with NEITHER age nor handedness set — confirm that
  whole line is simply absent (no empty gap, no "· " artifact).
- Check a player with NO signature shots — confirm the "SIGNATURE
  SHOTS" label and list are both completely absent, and the shield's
  position doesn't shift back to centered (it should still sit on the
  right, with more empty space on the left when there's nothing to
  show there).
- If any player has more than 5 signature shots, confirm the list
  shows exactly 5 plus a "+N more" line; if none do, this is a lower-
  priority check — note it and move on.
- Click the card and confirm the downloaded PNG matches the on-page
  layout, including the age/handedness line and signature shots.
- Confirm the card is still responsive on a narrow viewport (no
  horizontal scroll) — unrelated to this change, but a quick sanity
  check given the internal layout moved around.

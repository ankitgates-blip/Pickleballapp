# Player Stats Card Visual Redesign v2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild `PlayerStatsCard.tsx`'s visual markup to closely match
the approved reference (wing-flourished shield with a ball icon,
tier-count chevrons, skull+laurel for the top tier, a ribbon banner, a
"THREAT LEVEL: N" side-panel header) — purely visual, no data/prop
changes.

**Architecture:** One file rewrite. `PlayerStatsCardProps`, the
`handleDownload` PNG-export pipeline (including the data-URI photo fix),
and `loadPhotoDataUrl` are untouched — only the tier-lookup constants and
the JSX returned by the component change.
...
- No prop changes — `PlayerStatsCardProps` (13 fields) stays exactly as
  shipped; `people/[id]/page.tsx` is not touched by this plan.
- No changes to `handleDownload`, `loadPhotoDataUrl`, or any part of the
  PNG-export pipeline — the photo-embedding and responsive-CSS fixes
  already shipped must remain intact.
- Total Matches Played stays on the card (already present in the side
  panel's bullet list) — it is not shown in the reference image, but is
  an explicit prior requirement; it must not be silently dropped.
- Chevron count per tier: 1 for LOW THREAT, 2 for WATCH OUT, 3 for
  DANGEROUS, 3 for HIGH THREAT, 2 for DO NOT PLAY.
- Skull + laurel-branch flourish shows ONLY for the DO NOT PLAY tier.
- `RISK_LABELS` becomes unused by this redesign (the side panel no
  longer shows a "STATUS: X RISK" line) — remove it rather than leaving
  dead code.

---

### Task 1: Rebuild the card's visual markup

**Files:**
- Modify: `apps/organizer-web/app/components/PlayerStatsCard.tsx`

**Interfaces:**
- No interface changes — `PlayerStatsCardProps`, the default export
  signature, and everything the component consumes (`threatTierFor`,
  `formTierFor`, `sanitizeFileNamePart`) are identical to the shipped
  version.

This is a full-file replacement — the changes touch the tier-lookup
constants section, add two small new pure helpers, and replace the
entire `return (...)` JSX block. Rather than a sequence of fragile
find/replace edits across a file this interconnected, write the
complete file below.
...
(this task adds no new tests and touches no test-covered logic — the
whole diff is presentational JSX/SVG in a component with no dedicated
test file, per this project's established convention for UI components
like `ThreatBadge`).
...
git commit -m "feat: rebuild Player Stats Card shield graphic to match reference"
```
...
needed — this task touches no schema, and no props/page wiring changed.
...
- Open a player's `/people/[id]` page. Confirm the badge now shows a
  shield with wing flourishes, a small ball icon at the top, the
  correct number of chevron stripes for that player's tier, and a
  ribbon banner with the tier name below it.
- Confirm a DO NOT PLAY-tier player (or temporarily check one, if none
  currently qualify) additionally shows the skull + laurel-branch
  flourish beneath the chevrons — and that no other tier shows it.
- Confirm the side panel's header now reads "🚨 THREAT LEVEL: N" (N
  matching the main panel's Threat Level box) instead of the old
  generic "PLAYER STATUS" text, with a ribbon-style tier banner beneath
  it.
- Confirm Total Matches Played is still visible (in the side panel's
  bullet list) even though it's no longer in the main panel's bottom
  row.
- Click the card and confirm the downloaded PNG matches the on-page
  appearance (including the photo, still correctly embedded from the
  earlier fix) and is still responsive on a narrow viewport.
- Check a player with zero completed matches: confirm the card still
  renders cleanly (LOW THREAT tier, 1 chevron, no skull/laurel, "—" for
  trend) with no crash.
